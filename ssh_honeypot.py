...
cred_logger = logging.getLogger('CredLogger')
...
# Test log
# funnel_logger.info("User signed in")
    ...
    while True:  
        ...
        if char == b"\r":
            if command.strip() == b'exit':
                ...
                response = b"\n" + b"\\usr\\local" + b"\r\n"
                cred_logger.info(f'Command {command.strip()}' + "executed by " f'{client_ip}')
...
def honeypot(address, port, username, password):
    ...
    while True:
        try:
            ...
            ssh_honeypot_thread = threading.Thread(target=client_handel,args=(client, addr, username, password))
